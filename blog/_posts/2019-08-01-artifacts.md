...
Artifacts can be manipulated using convenient APIs exposed from the `Pkg.Artifacts` namespace.  As a motivating example, let us imagine that we are writing a package that needs to load the [Iris machine learning dataset](https://archive.ics.uci.edu/ml/datasets/iris).  While we could download the dataset to a file within the package directory, this is undesirable because it (a) modifies state within a package directory that may be read-only and (b) is not shared across multiple versions of our package.  Instead, we will first check to see if our `iris` artifact exists on-disk and if it does not, we will create it by downloading the appropriate file, then bind the result into our `Artifact.toml` file:
...
The autogenerated packages are true Julia packages, with the notable inclusion of `Artifact.toml` files that download the appropriate version of whatever binary artifacts were built by `BinaryBuilder` and uploaded to the repository's GitHub release.  BinaryBuilder attempts to upload all packages to `JuliaBinaryWrappers/$(package_name)_jll.jl`, however this is, of course, configurable.  An example package [is given here](https://github.com/JuliaBinaryWrappers/c_simple_jll.jl), the most interesting part being the new API that is exposed by these autogenerated packages.
